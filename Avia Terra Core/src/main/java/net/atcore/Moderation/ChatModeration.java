...
public class ChatModeration {
    ...
    private static final float MAX_PUNTOS = 50;
    ...
    public static boolean antiSpam(Player player, String message) {
        ...
        if (puntos < 0) {//si su puntos son negativos se lo hace saber
            ...
            float second = (puntos*0.5F)/20F;//formula para calcular el tiempo que le fata para volver a escribir
            sendMessage(player, "mensaje eliminado por Spam espera <|" + Math.abs(second) + "|> segundos", TypeMessages.ERROR);
            return true;
        }else {
            puntos = puntos - (message.length()*1.5F) - 15;//por cada letra más puntos le resta y por cada mensaje resta 15
            if (puntos < 0) {//si los puntos llega a estar en negativo lo multiplica por la cantidad de veces que fue penalizado en 2 minutos
                counts.put(uuid ,counts.getOrDefault(uuid, 1) + 2);//aumenta la penalización
    ...
    public static void tickEvent() {
        new BukkitRunnable() {
            @Override
            public void run() {
                puntosDeChat.replaceAll((uuid, punto) -> punto > MAX_PUNTOS ? punto : punto + 1);
            }
        }.runTaskTimer(AviaTerraCore.getInstance(), 1, 2);
