...
public enum Message {
    ...
    LOGIN_PREMIUM_VALIDATION_OK("login","Certificación del premíum valida del jugador <|%s|>", INFO),
    LOGIN_PREMIUM_VALIDATION_FAILED_LOG_0("login", "La ip que se envío el paquete no es la misma que se envío el primer paquete por el jugador <|%1$s|> y la ip <|%2$s|>. Discrepancia detectada", TypeMessages.INFO),
    LOGIN_PREMIUM_VALIDATION_FAILED_LOG_1("login", "Los datos dados por mojang no concuerda con el jugador <|%1$s|> y la ip <|%2$s|>. Discrepancia detectada", TypeMessages.INFO),
    LOGIN_PREMIUM_VALIDATION_FAILED_LOG_2("login", "No se encontró registros en mojang del jugador <|%1$s|> y la ip <|%2$s|>. Discrepancia detectada", TypeMessages.INFO),
    LOGIN_PREMIUM_VALIDATION_FAILED_LOG_3("login", "hubo un error al activar el protocolo de encriptación por el jugador <|%1$s|> y la ip <|%2$s|>. Discrepancia detectada", TypeMessages.INFO),
    LOGIN_PREMIUM_VALIDATION_FAILED_LOG_4("login", "tokens no son iguales del el jugador <|Desconocido|> y la ip <|%1$s|>. Discrepancia detectada", TypeMessages.INFO),
    ...
    LOGIN_LIMBO_INITIATED_LOG("login", "El jugador <|%s|> entro en modo limbo", INFO),
    LOGIN_LIMBO_EXIT("login", "El jugador <|%s|> salio del modo limbo", INFO),
    ...
    LOGIN_TWO_FACTOR_SEND_CODE_GMAIL_LOG("login", "Se envió código de verificación a <|%1$s|> (%2$s)", INFO),
    LOGIN_TWO_FACTOR_SEND_CODE_DISCORD_LOG("login", "Se envió código de verificación a <|%1$s|> (%2$s)", INFO),
    SECURITY_KICK_ANTI_TWO_PLAYER("security", "Hay un jugador con tu nombre", KICK),
    SECURITY_REMOVE_OP("security", "El jugador <|%s|> tenia creativo o Op y fue eliminado", WARNING),
    ...
    DATA_BAN_ADD_OK("data.ban", "El jugador <|%1$s|> fue baneado de <|%2$s|> durante <|%3$s|> por <|%4$s|> y la razón es <|%5$s|>", SUCCESS),
    DATA_BAN_ADD_FAILED("data.ban", "Error al banear al jugador <|%1$s|> Contexto: <|%2$s|> Tiempo de baneo: <|%3$s|> Autor: <|%4$s|> razón de baneo: <|%5$s|>", ERROR),
    DATA_BAN_REMOVE_OK("data.ban","Se Desbano el jugador <|%1$s|> en el contexto <|%2$s|> por <|%3$s|>", SUCCESS),
    DATA_BAN_REMOVE_FAILED("data.ban", "Error al desbanear al jugador <|%1$s|> del contexto <|%2$s|> por <|%3$s|>", ERROR),
    DATA_REGISTER_DATE_OK("data.register", "Se actualizó la <|fecha del ultimo login|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_DATE_FAILED("data.register", "No se pudo actualizar la <|fecha del ultimo login|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_GMAIL_OK("data.register", "Se actualizó el <|gmail|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_GMAIL_FAILED("data.register", "No se pudo actualizar el <|gmail|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_DISCORD_OK("data.register", "Se actualizó el <|discord|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_DISCORD_FAILED("data.register", "No se pudo actualizar el <|discord|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_PASSWORD_OK("data.register", "Se actualizó la <|contraseña|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_PASSWORD_FAILED("data.register", "No se pudo actualizar la <|contraseña|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_ADDRESS_OK("data.register", "Se actualizó la <|ip|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_ADDRESS_FAILED("data.register", "No se pudo actualizar la <|ip|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_STATE_OK("data.register", "Se actualizó la <|estado de la cuenta|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_STATE_FAILED("data.register", "No se pudo cambiar el <|estado de la cuenta|> del jugador <|%s|>", ERROR),
    DATA_REGISTER_REMOVE_OK("data.register", "Se borro el <|registro|> del jugador <|%1$s|> por <|%2$s|>", SUCCESS),
    DATA_REGISTER_REMOVE_FAILED("data.register", "Hubo un error al borrar el <|registro|> del jugador <|%1$s|> por <|%2$s|>", ERROR),
    DATA_REGISTER_UUID_BEDROCK_OK("data.register", "Se añadió la <|UUID de bedrock|> del jugador <|%s|> exitosamente", SUCCESS),
    DATA_REGISTER_UUID_BEDROCK_FAILED("data.register", "Hubo un error al añadir <|UUID de bedrock|> del jugador <|%s|> exitosamente", ERROR),
    ...
    COMMAND_LINK_ALREADY_GMAIL("command.link", "Ya tiene un correo vinculado solo usar el comando para vincular un nuevo correo", WARNING),
    ...
    COMMAND_LINK_MISSING_ARGS_GMAIL("command.link", "Tiene poner tu gmail o el código de validación", ERROR),
    ...
    COMMAND_LINK_NOT_FOUND_GMAIL("command.link", "No tiene un Gmail vinculado", ERROR),
    ...
    COMMAND_HOME_MAX_HOME("command.home", "Solo se permite máximo 5 homes",ERROR),
    ...
    COMMAND_HOME_IS_NOT_ALPHABETICAL("command.home", "el nombre del home tiene que comenzar con una letra", ERROR),
